...
X.InstructionSet = (function() {
  ...
  var instruction_implementations = {
    ...
    ADD: function(parameters, parameter_names) {
      switch (parameter_names[0]) {
        ...
        return function(parameters, operands) {
          ...
          X.CPU.zero = false;
          X.CPU.addsub = false;
          X.CPU.halfcarry = (a & 0xF) + (b & 0xF) > 0xF; // TODO not sure
          X.CPU.carry = (a & 0xFF) + (b & 0xFF) > 0xFF; // TODO
        };
    ...
    LDHL: function(parameters, parameter_names) {
      return function(parameters, operands) {
        X.CPU.HL = X.Utils.wrap16(X.CPU.SP + parameters[1].get(operands));
        X.CPU.zero = false;
        X.CPU.addsub = false;
        // TODO halfcarry, carry
      };
    },
    ...
    PUSH: function(parameters, parameter_names) {
      return function(parameters, operands) {
