...
X.InstructionSet = (function() {
  ...
  function ZeroPageParameter(parameter) { this.parameter = parameter; }
  X.Utils.inherit(ZeroPageParameter, Parameter);
  ZeroPageParameter.prototype.get = function(operands) { return 0xFF00 + this.parameter.get(operands); };
  ZeroPageParameter.prototype.to_string = function() { return '[' + this.constructor.name + ', ' + this.parameter.to_string() + ']'; };
  ...
  function generate_parameter(opcode, parameter_name) {
    ...
    if (parameter_name == 'SP+d8') { // Weird LD
      return {
        get: function(operands) { return X.CPU.SP + operands[0]; },
        to_string: function() {return 'todoooo'} // TODO
      };
    }
    ...
    if (core.match(/^[A-Z]+$/)) { // Register
      ...
      parameter = new ZeroPageParameter(sub_parameter);
    }
  ...
  var instruction_implementations = {
    ...
    ADD: function(parameters, parameter_names) {
      switch (parameter_names[0]) {
        ...
        return function(parameters, operands) {
          var a = X.CPU.SP;
          var b = parameters[1].get(operands);
          var x = X.Utils.wrap16(a + b); // TODO signed ??????
          X.CPU.SP = x;
          X.CPU.zero = false;
          X.CPU.addsub = false;
    ...
    LDD: function(parameters, parameter_names) {
      var ld = this.LD(parameters, parameter_names);
      return function(parameters, operands) {
